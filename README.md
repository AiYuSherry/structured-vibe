...
- 活跃 PRD、Changelog、Dev-Log、Todo List、测试清单统一使用 `YYMMDD` 文件名前缀。
- 每轮改动前先检查日期；跨天继续开发时，先把活跃文档重命名到今天前缀，再写新内容。
- 如果项目 Todo 使用「长期待办 / 每日待办」结构，完成长期待办时要同步把原提出日期的每日待办打勾并补完成结果。
